...
public class PeakFareStrategy implements IFareStrategy {
    ...
    public float CalculateFare(Ticket ticket) {
        ...
        float totalPrice = (float) (hours * 2 * ticket.getParkingSpot().getPeakPrice());

        return totalPrice + (minutes > 30 ? 2 : 1) * ticket.getParkingSpot().getPeakPrice();
    }
}
